...
case "$command_id" in
  ...
  host-maintenance)
    ...
    elif [ "$#" -eq 1 ] && [ "$1" = "--prune-builder-cache" ]; then
      exec /bin/bash scripts/production-maintenance.sh --prune-builder-cache
    fi
    block "host-maintenance accepts only optional --check-only or --prune-builder-cache."
    ;;
  observe)
    [ "$#" -eq 1 ] && { [ "$1" = "ps" ] || [ "$1" = "logs" ]; } || \
