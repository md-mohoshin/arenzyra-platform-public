...
test("automatic capacity preparation runs only for build-producing routine modes", () => {
  ...
  assert.match(
    deploy,
    /if \[ "\$MODE" = "full" \] \|\| \[ "\$MODE" = "discord-bot" \] \|\| \\\s+\[ "\$MODE" = "api-recovery" \] \|\| \[ "\$MODE" = "web-recovery" \]; then\s+verify_production_activation_boundary\s+if \[ "\$INTERRUPTED_FULL_RESUME" -eq 1 \]; then[\s\S]*verify_inherited_lock_held[\s\S]*else\s+ARENZYRA_DEPLOY_LOCK_INHERITED=1 \\\s+bash scripts\/prepare-production-deploy-capacity\.sh\s+fi/,
  );
