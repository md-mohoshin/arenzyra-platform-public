      ...
      "WITH envelope_constraint AS (
       ...
       SELECT
         ...
         (SELECT count(*) FROM \"DiscordIdpSchedule\"
          WHERE \"roomPassword\" !~ '\''^v1:[A-Za-z0-9_-]{16}:[A-Za-z0-9_-]{22}:[A-Za-z0-9_-]*$'\''),
         COALESCE(
...
if ! [[ "$idp_counts" =~ ^[0-9]+[[:space:]][0-9]+[[:space:]][0-9]+[[:space:]](-|[0-9a-f]+)$ ]]; then
  printf 'IDP ENCRYPTION GATE BLOCKED: invalid database verification result.\n' >&2
  exit 75
fi
read -r migration_applied_count constraint_count legacy_count constraint_definition_hex <<<"$idp_counts"

node scripts/verify-idp-credential-storage.cjs \
