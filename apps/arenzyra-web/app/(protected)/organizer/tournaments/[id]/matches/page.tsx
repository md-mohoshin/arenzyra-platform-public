...
function toScheduleDateValue(value?: string | null) {
  if (!value) return "";
...
function statusBadgeClass(status: string) {
  switch (status) {
    ...
    case "FINALIZING":
      return "border-amber-500/20 bg-amber-500/20 text-amber-300";
    case "FINALIZED":
      return "border-blue-500/20 bg-blue-500/20 text-blue-400";
    case "PAUSED":
    case "COUNTDOWN":
      return "border-violet-500/20 bg-violet-500/20 text-violet-300";
    case "UPCOMING":
    default:
...
function formatMatchSchedule(match: MatchRow) {
  const scheduledDate = toScheduleDateValue(match.scheduledAt);
  if (scheduledDate) {
  ...
  const fallback = match.startedAt ?? match.endedAt ?? match.createdAt ?? null;
  if (!fallback) return "--";
...
function MatchesPageInner() {
  ...
  const activeDateFilter =
    dateFilter === "all" || dateOptions.includes(dateFilter) ? dateFilter : "all";
  ...
  const visibleMatches = useMemo(() => {
    ...
    return sorted.filter((match) => {
      if (
        ...
        toScheduleDateValue(match.scheduledAt) !== activeDateFilter
      ) {
        return false;
      }
      ...
      return (
        statusFilter === "all" ||
        (statusFilter === "ended"
          ? ["FINALIZING", "FINALIZED"].includes(normalizeStatus(match))
          : normalizeStatus(match).toLowerCase() === statusFilter)
      );
    });
  }, [activeDateFilter, matches, statusFilter]);
      ...
      <div className="flex flex-wrap gap-3 rounded-xl border border-white/10 bg-slate-900/50 p-3">
        ...
        <label className="ml-auto flex items-center gap-3 text-sm text-white/80">
          ...
          <select
            ...
            className="rounded-lg border border-white/15 bg-black/40 px-3 py-2 text-white outline-none transition focus:border-cyan-400"
          >
            <option value="all">All dates</option>
            {dateOptions.map((dateValue) => (
              <option key={dateValue} value={dateValue}>
                {formatMatchSchedule({ id: "", scheduledAt: dateValue })}
              </option>
            ))}
          </select>
        </label>
