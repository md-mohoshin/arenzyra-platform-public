...
export default function TournamentCreatePage() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    setSubmitting(true);
      ...
      const logoUrl = ensureApiUrl(logoUrlRaw);
      const bannerUrl = ensureApiUrl(bannerUrlRaw ?? logoUrlRaw);
      ...
      const res = await apiFetch("/organizer/tournaments", {
        method: "POST",
        body: JSON.stringify(payload),
      });

      const json = await res.json();
      const id = json?.id ?? json?.data?.id ?? json?.tournament?.id;
      if (id) {
        router.push(`/organizer/tournaments/${id}`);
      } else {
        throw new Error("Tournament created but id not returned.");
      }
    } catch (err) {
      const message =
    ...
    <div className="flex flex-col h-full px-4">
      <div className="space-y-6 py-6">
        <div className="mb-0">
          ...
          <p className="text-sm text-white/65">Quick setup for a new tournament.</p>
        </div>
