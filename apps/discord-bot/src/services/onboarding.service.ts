  ...
  },
  {
    ...
    messages: [
      [
        ...
        "5. Use `/session-admin registration-state` when staff must override the registration schedule.",
        "6. Use `/session-admin channel-state state:Paused` before manual emergency edits, then set it to Active when the bot should resume.",
  ...
  },
  {
    ...
    messages: [
      [
        ...
        "`/register team`, `/register tournament` - Register through structured Discord options in the synced registration channel.",
        "`/session-admin` - Pause/resume a channel and manage registration, waitlist, IDP, slot-response, and result-summary state.",
        "`/team-manager add|remove` - Update managers from the configured transfer channel.",
        "`/team-media logo|photo` - Upload media in synced scrim/event session channels only; production channels continue using `%logo` and `%photo`.",
        "`/slot confirm|free` - Confirm an assigned slot or check availability.",
        "`/idp-broadcast` - Send a structured IDP update and attachments to registered managers.",
      ...
      [
        "**Legacy Channel Text Commands (kept during migration)**",
        "",
        ...
        "`%logo ...` - Attach or update a team logo; keep using this in production logo channels.",
        "`%photo ...` - Attach or update player photos; keep using this in production player-photo channels.",
  ...
  },
  {
    ...
    messages: [
      [
        ...
        "1. Run `/session-admin channel-state state:Paused` in the affected synced channel.",
        "2. Fix the session, team, role, schedule, or ban rule in the web app.",
        "3. Re-sync or refresh from the web app when needed.",
        "4. Run `/session-admin channel-state state:Active` in the affected synced channel.",
