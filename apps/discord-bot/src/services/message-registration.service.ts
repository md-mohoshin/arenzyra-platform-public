...
export class MessageRegistrationService {
  ...
  private idpDmEnabled(config: SessionDiscordConfigResponse | null | undefined) {
  ...
  private async resolveIdpDmChannel(
    ...
    const key = this.idpDmChannelCacheKey(
      message.guild.id,
      message.channel.id,
    );
  ...
  private discordUserBanLabel(
    ...
    if (
      cleanDiscordUserId &&
      mentionableUserIds.has(cleanDiscordUserId)
    ) {
      return `<@${cleanDiscordUserId}>`;
    }
  ...
  private async handleIdpDmStateCommandMessage(
    ...
    if (!message.guild) {
      await message.reply("Use `%dm on` or `%dm off` inside the synced IDP channel.");
      return true;
    }
  ...
  private async handleIdpDmForwardMessage(
    message: Message,
  ): Promise<boolean> {
    const content = message.content.trim();
    if (
  ...
  private async handleIdpDmReplyButton(
    ...
    if (interaction.user.id !== target.managerDiscordUserId) {
      await interaction.reply({
        content: "This reply button is only for the manager who received this DM.",
        ephemeral: true,
      });
      return true;
    }
  ...
  private async handleIdpDmReplyModal(
    ...
    if (interaction.user.id !== target.managerDiscordUserId) {
      await interaction.reply({
        content: "This reply form is only for the manager who received this DM.",
        ephemeral: true,
      });
      return true;
    }
      ...
      const {
        ...
        async () => {
          ...
          if (parsed.action === "final") {
            ...
            const postedResultsResult = await this.postResultToConfiguredChannel(
              interaction,
              overallResultChannelId,
              finalResult,
              {
  ...
  private reviewedRowLine(row: ReviewedResultRow, index: number) {
    ...
    const confidence =
      typeof row.confidence === "number" && Number.isFinite(row.confidence)
        ...
        : "";
    const edited = row.edited ? " edited" : "";
    return `${mark} ${index + 1}) P${row.position} ${row.tag} ${slot} - ${row.kills} kills${players}${ocrTag}${evidence}${confidence}${edited}${reason}`;
  }
  ...
  private paginateAutoResultDetailLines(lines: string[]) {
    ...
    for (const line of lines) {
      ...
      if (current.length > 0 && nextLength > AUTO_RESULT_DETAIL_PAGE_MAX_CHARS) {
        pages.push(current);
        current = [line];
  ...
  private formatAutoResultDashboard(pending: AutoResultPending) {
    ...
    const autoSkippedCount = pending.rows.filter((row) => row.autoSkipped).length;
    const issues = this.reviewIssues(pending);
    const missingOfficialSlots = this.missingOfficialSlots(pending);
    ...
    const lines = [
      ...
      autoSkippedCount
        ? `Auto-skipped duplicates: ${autoSkippedCount}`
        : null,
      "",
      "1 Preview details",
      "2 Apply result",
