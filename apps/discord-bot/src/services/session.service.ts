...
export class DiscordSessionService {
  ...
  private defaultProductionDiscordSet(index: number): ProductionDiscordSet {
    const safeIndex = Math.min(20, Math.max(1, index));
    return {
      key: this.productionDiscordSetKey(safeIndex),
      index: safeIndex,
      setName: `set-${safeIndex}`,
      eventId: null,
      eventName: null,
      categoryId: null,
      categoryName: null,
      ...
      productionRoleName: `Production ${safeIndex}`,
      startSlot: null,
      normalSlots: null,
      vipSlots: null,
      slots: [],
  ...
  private logoNamesLookRelated(requestedNameKey: string, savedNameKey: string) {
  ...
  private noShowAutoBanRuleType(
    ...
    const raw = (
      ...
      .toUpperCase()
      .replace(/[\s-]+/g, "_");
    if (raw === "MATCH_MISSED" || raw === "SPECIFIC_MATCH" || raw === "MATCH") {
      return "MATCH_MISSED";
    }
  ...
  private async validatedManagerMentionIds(
    ...
    const cleanDiscordUserIds = this.uniqueStrings(
      discordUserIds.filter((discordUserId): discordUserId is string =>
  ...
  private latestEditableMatchBackups(
    ...
    const ordered = backups
      .filter(
        (backup) =>
          backup.kind?.toUpperCase() === "MATCH" && (backup.rowCount ?? 0) > 0,
      )
      .slice()
      .sort((left, right) => {
  ...
  private async reconcileTeamSetAccessRoles(
    ...
    if (affectedTeamIds.length === 0 && explicitDiscordUserIds.length === 0) {
      return;
    }
    const managedRoleIds = this.scrimManagedRoleIds(setup);
  ...
  async registerTeamAndJoinScrim(
    ...
    try {
      ...
      await this.assertDiscordRegistrationAllowed(
        ...
        {
          staffBypass,
          registrationWindowBypass: options.registrationWindowBypass === true,
