...
test("remote live widget capabilities strip network-visible credential query keys", async () => {
  const capability = `wgt_${Buffer.alloc(32, 27).toString("base64url")}`;
  const instance = await startServer(false, {
    resolveApiBase: () => "https://api.arenzyra.com",
    resolveWidgetContext: async ({ instanceKey }) => {
      assert.equal(instanceKey, capability);
      return {
        id: "instance-private-query",
        widgetKey: "final-five-alive",
        match: { id: "match-private-query" },
        organization: {
          id: "org-private-query",
          slug: "private-query",
          name: "Private Query",
        },
      };
  ...
  try {
    const response = await fetch(
      `${instance.baseUrl}/w/${encodeURIComponent(capability)}?matchAccessKey=leak&MATCHACCESSKEY=case-leak&access_token=leak&ACCESS_TOKEN=case-leak&token=leak&TOKEN=case-leak`,
    );
    assert.equal(response.status, 200);
    const html = await response.text();
    const iframeSource = html
    ...
    assert.deepEqual(
      [...new URLSearchParams(target.hash.slice(1)).entries()],
      [["matchAccessKey", capability]],
    );
    assert.equal(target.href.split("#", 1)[0].includes(capability), false);
  } finally {
    await stopServer(instance);
  }
});
...
test("retired Web widget keys cannot select a remote-live renderer", async () => {
  ...
  try {
    for (const retiredWidgetKey of retiredWidgetKeys) {
      ...
      assert.doesNotMatch(html, new RegExp(`/widgets/${retiredWidgetKey}`));
    }
  } finally {
    await stopServer(instance);
  }
