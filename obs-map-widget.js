(function () {
  ...
  function normalizeTeamBrandingRecord(record, index) {
    ...
    return {
      teamId,
      slot,
      teamName: teamName || teamTag || (slot === null ? `Team ${index + 1}` : `Team ${slot}`),
      teamTag,
  ...
  const state = {
    ...
    commentaryFlags: {
      showPanel: resolveQueryFlag("commentary", true),
    },
    commentaryMarkup: "",
    commentaryMetaLabel: "",
    debugFlags: {
    ...
    debugMarkup: "",
    distanceFlags: {
      showPanel: resolveQueryFlag("distances", true) && resolveQueryFlag("teamdistance", true),
    },
    distanceMarkup: "",
    distanceMetaLabel: "",
    frame: {
    ...
    showLegend: resolveQueryFlag("legend", true),
  ...
  function worldToBasePixelY(worldY, mapDefinition, height, options) {
    const worldSize = resolveWorldSize(mapDefinition);
    const renderBounds = getMapRenderBounds(mapDefinition);
    const normalizedFromTop = 1 - normalizeWorldY(worldY, mapDefinition, options) / worldSize;
  ...
  function isCircleZoomActive(zone) {
    if (!zone) {
      return false;
    }
  ...
  function updateMapCamera(mapDefinition, circle, visiblePlayers, width, height, now) {
    ...
    const shouldZoom = camera.enabled && isCircleZoomActive(state.zone);
    const target =
  ...
  function buildCommentaryTeamLabel(source) {
    ...
    if (slot !== null && displayName && displayName !== `Team ${slot}`) {
  ...
  function drawFrame(now) {
    ...
    const zoneShadeCircle = blueCircle || zoneAnimation.circle;
